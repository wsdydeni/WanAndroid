...
class SearchActivity : BaseVMActivity<ActivitySearchBinding,SearchViewModel>(R.layout.activity_search) {
    ...
    override fun initView() {
        ...
        search_edit.setOnEditorActionListener { _, actionId, _ ->
            if(actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                ARouter.getInstance().build("/search/DetailActivity").withString("text",text).navigation()
    ...
    override fun initData() {
        hotBinder = HotBinder().apply {
            setOnClick {
                ARouter.getInstance().build("/search/DetailActivity").withString("text",it).navigation()
            }
        }
